...
public class BarChartProcessor<Model> implements ChartProcessor<Model, Map<String, Double>> {
    ...
    private Collector<Model, ? , Double> collector;

    public BarChartProcessor(Function<Model, String> field, Collector<Model, ?, Double> collector) {
        this.field = field;
    ...
    public Chart<Map<String, Double>> process(List<Model> sourceData) {
        return new Chart<>(sourceData.stream().collect(groupingBy(field, collector)));
    }
}
